...
Byte encoding of unicode text. Basically converts the text into binary representation. More here - https://en.wikipedia.org/wiki/UTF-8.  

Utf-8 is by far the most popular and backward compatible representation (with ASCII as well).

We can't to convert to byte format because unicode is an thriving and changing format and byte helps us keep locked down to the right representation.
...
We use list so that we can get the raw bytes and not the text representation


```python
print(len("안녕하세요 👋 (hello in Korean!)"))
print(len(list("안녕하세요 👋 (hello in Korean!)".encode("utf-8"))))
```
...
Note: There are more encoding digits than there are characters in the original string.

That is because some of the characters, specially the non-english one have more than 1 number assigned to them.
...
```
    ...
    <ipython-input-13-548243396b12> in <cell line: 1>()
    ----> 1 b'\x88'.decode('utf-8')
    

    UnicodeDecodeError: 'utf-8' codec can't decode byte 0x88 in position 0: invalid start byte


So you cannot break the each byte seperately. All the 3 bytes together make up that strange korean character.

Let's look at Hindi text also


```python
hindi_consonants = ['क','ख','ग','घ','ड़','च','छ','ज','झ','ञ',
                    'ट','ठ','ड','ढ','ण','त','थ','द','ध','न',
                    'प','फ','ब','भ','म','य','र','ल','व','श',
                    'ष','स','ह','क्ष','त्र','ज्ञ','श्र']
```

Some hindi consonants do not convert to proper unicode format


```python
for x in hindi_consonants:
  try:
    print(ord(x))
  except:
    print(x)
...
That is because those consonants are made up of 2 or more things
...
```python
[list(x.encode("utf-8")) for x in hindi_consonants]
```
    ...
    [[224, 164, 149],
     ...
     [224, 164, 184],
     [224, 164, 185],
     [224, 164, 149, 224, 165, 141, 224, 164, 183],
     [224, 164, 164, 224, 165, 141, 224, 164, 176],
     [224, 164, 156, 224, 165, 141, 224, 164, 158],
     [224, 164, 182, 224, 165, 141, 224, 164, 176]]



# BytePair Encoding

Going with raw byte encoding gives us small vocabulary (atleast for most of the commonly used text). But for even short lines of text the number of tokens can be very large. As we know the Language Models have limited context length they can optimally use. So we want to be judicious in terms of the tokens we generate for a text and the vocabulary. That is where bytepair encoding comes in.
...
A function that takes in a list of tokens, goes through each pair of tokens, and updates the number of times tha pair occurs in a dictionary
...
We can find that the pair that occurs the maximum times is (101,32), and it occurs 20 times
...
Now, in bytepair encoding, we want to merge the most commonly occuring pairs iteratively, and keep adding those merged pairs into our vocabulary. We stop when we reach a particular vocabulary size.

First create a function to merge the pair in a given list of ids, and replace it with new id idx. And return the new list of ids -> newids with the pair of ids replaced with a new idx.
...
Run merge 20 times for the most commonly pairs occuring in the text
...
```python
tokens = b"".join(vocab[idx] for idx in [0,1,2])
tokens
```
...
```python
vocab = {idx: bytes([idx]) for idx in range(256)} # first 256 integers are encoded to there resepctive bytes, for english alphabets
for (p0, p1), idx in merges.items(): # from 256 onwards we assign it to the new merged tokens, where key is the integer and value is the combined encoding
    vocab[idx] = vocab[p0] + vocab[p1] # b'\x00 + b'\x01 = b'\x00\x01'

def decode(ids):
  # given ids (list of integers), return Python string
  tokens = b"".join(vocab[idx] for idx in ids) # concatenate all the byte encodings together for all the integers in the input
  text = tokens.decode("utf-8", errors="replace") # decode it all together into a single string
  return text
...
```python
# prompt: run decode on a list of integers from 0 to 10
ids = list(range(60,70))
print([decode([id]) for id in ids])
...
def encode(text):
  ...
  # merge tokens based on the values in the get_stats and merges
  while len(tokens) >= 2:
    stats = get_stats(tokens)
...
```
    ...
    --2024-02-28 10:36:42--  https://openaipublic.blob.core.windows.net/gpt-2/models/1558M/vocab.bpe
    ...
    vocab.bpe           100%[===================>] 445.62K  2.73MB/s    in 0.2s    
    
    2024-02-28 10:36:43 (2.73 MB/s) - ‘vocab.bpe’ saved [456318/456318]
    
    --2024-02-28 10:36:43--  https://openaipublic.blob.core.windows.net/gpt-2/models/1558M/encoder.json
    Resolving openaipublic.blob.core.windows.net (openaipublic.blob.core.windows.net)... 20.60.179.33
    Connecting to openaipublic.blob.core.windows.net (openaipublic.blob.core.windows.net)|20.60.179.33|:443... connected.
    HTTP request sent, awaiting response... 200 OK
    ...
    encoder.json        100%[===================>]   1018K  4.27MB/s    in 0.2s    
    
    2024-02-28 10:36:43 (4.27 MB/s) - ‘encoder.json’ saved [1042301/1042301]
